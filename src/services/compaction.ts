...
function createCompactionPrompt(projectMemories: string[]): string {
  const memoriesSection =
    projectMemories.length > 0
      ? `
## Project Knowledge (from Unison Brain)
  ...
  client: {
    session: {
      ...
      messages: (params: {
        path: { id: string };
        query: { directory: string };
      }) => Promise<{ data?: Array<{ info: MessageInfo }> }>;
      promptAsync: (params: {
        path: { id: string };
...
export function createCompactionHook(
  ...
  async function fetchProjectMemoriesForCompaction(): Promise<string[]> {
    try {
      const result = await brainClient.listMemories(tags.project, CONFIG.maxProjectMemories);
      if (!result.success) return [];
      return result.memories
        .map((doc) => doc.tldr ?? doc.title ?? "")
        .filter(Boolean);
    } catch (err) {
  ...
  async function saveSummaryAsMemory(sessionID: string, summaryContent: string): Promise<void> {
    ...
    try {
      const result = await brainClient.addMemory(
        `[Session Summary]\n${summaryContent}`,
        tags.project,
        { kind: "conversation" }
      );
      ...
      if (result.success) {
        log("[compaction] summary saved as memory", { sessionID, path: result.path });
      } else {
  ...
  async function checkAndTriggerCompaction(
    ...
    await ctx.client.tui
      .showToast({
        body: {
          ...
          message: `Context at ${(usageRatio * 100).toFixed(0)}% — compacting with Unison brain context...`,
          variant: "warning",
          duration: 3000,
        },
      })
    ...
    try {
      ...
      await ctx.client.tui
        .showToast({
          body: {
            ...
            message: "Session compacted with Unison brain context. Resuming...",
            variant: "success",
            duration: 2000,
          },
        })
      ...
      setTimeout(async () => {
        try {
          const mdir = getMessageDir(sessionID);
          const stored = mdir ? findNearestMessageWithFields(mdir) : null;
  ...
  async function handleSummaryMessage(
    sessionID: string,
    _messageInfo: MessageInfo
  ): Promise<void> {
    log("[compaction] handleSummaryMessage called", {
      sessionID,
      inSet: state.summarizedSessions.has(sessionID),
    });
